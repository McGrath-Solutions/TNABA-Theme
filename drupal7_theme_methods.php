...
$art_style = '';
$art_head = '';
function tnaba4_process_html(&$variables) {
...
function tnaba4_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];

  if (!empty($breadcrumb)) {
    // Provide a navigational heading to give context for breadcrumb links to
    // screen-reader users. Make the heading invisible with .element-invisible.
    $output = '<h2 class="element-invisible ms-postheader">' . t('You are here') . '</h2>';

    $output .= '<div class="breadcrumb ms-postcontent">' . implode(' | ', $breadcrumb) . '</div>';
    return $output;
  }
}
 ...
 * @ingroup themeable
 */
function tnaba4_button($variables) {
  $element = $variables['element'];
  $element['#attributes']['type'] = 'submit';
  element_set_attributes($element, array('id', 'name', 'value'));

  $element['#attributes']['class'][] = 'form-' . $element['#button_type'] . ' ms-button';
 ...
 */
function tnaba4_preprocess_page(&$vars) {
  global $art_style, $art_head;
 ...
 * @ingroup themeable
 */
function tnaba4_menu_local_task($variables) {
  ...
  //added ms-class
  $link['localized_options']['attributes']['class'] = array('ms-button');
 ...
 */
function tnaba4_feed_icon($variables) {
  $text = t('Subscribe to @feed-title', array('@feed-title' => $variables['title']));
  return l(NULL, $variables['url'], array('html' => TRUE, 'attributes' => array('class' => array('feed-icon', 'ms-rss-tag-icon'), 'title' => $text)));
}
 ...
 * @ingroup themeable
 */
function tnaba4_node_preview($variables) {
  $node = $variables['node'];
...
if (!empty($terms)) {
ob_start();?>
<span class="ms-posttagicon"><?php
$result .= ($result == '') ? ob_get_clean() : '&nbsp;|&nbsp;' . ob_get_clean();
$result .= '<div class="ms-tags">' . render($terms) . '</div>';
ob_start();?>
</span><?php
$result .= ob_get_clean();
}
...
ob_start();?>
<span class="ms-postcommentsicon"><?php
$output .= ob_get_clean();
$output .= $link_output;
$index++;
ob_start();?>
...
ob_start();?>
<span class="ms-postcategoryicon"><?php
$output .= ob_get_clean();
$output .= $link_output;
$index++;
ob_start();?>
...
*/
function tnaba4_form_alter(&$form, &$form_state, $form_id) {
...
function art_label($variables) {
  ...
  $attributes['class'] = $element['#type'] == 'checkbox' ? 'ms-checkbox' : ($element['#type'] == 'radio' ? 'ms-radiobutton' : '');

  if (!empty($element['#id'])) {
    $attributes['for'] = $element['#id'];
  }

  $result = array('label_head' => ' <label' . drupal_attributes($attributes) . '><span>',
    'label_title' => $t('!title !required', array('!title' => $title, '!required' => $required)),
    'label_tail' => "</span></label>\n",);
  return  $result;
}

function tnaba4_form_element_label($variables) {
...
function tnaba4_form_element($variables) {
  $element = &$variables['element'];
  // This is also used in the installer, pre-database setup.
  $t = get_t();
