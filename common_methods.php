...
function art_menu_xml_parcer($content, $show_sub_menus, $menu_class) {
  ...
  foreach($ul_elements as $ul) {
    // First ul element with css-class ms-hmenu or ms-vmenu
    if (($ul->getAttribute('class') == "ms-vmenu") || ($ul->getAttribute('class') == "ms-hmenu"))
        break;
    continue;
  }
...
function art_submitted_worker($date, $author) {
  ...
  if ($date != '') {
    ob_start();?>
<span class="ms-postdateicon"><?php
$output .= ob_get_clean();
$output .= $date;
ob_start();?>
  ...
  if ($author != '') {
    ob_start();?>
<span class="ms-postauthoricon"><?php if ($output != '') {
$output .= '&nbsp;|&nbsp;';
}
$output .= ob_get_clean();
$output .= $author;
...
function art_hmenu_output($content) {
  $menu = art_menu_worker($content, true, 'ms-hmenu');
...
function art_vmenu_output($subject, $content) {
  ...
  $result = '';
  $vmenu =	art_menu_worker($content, true, 'ms-vmenu');
  $result .= <<< EOT
<div class="ms-vmenublock clearfix">
        
EOT;
if (!empty($subject)) {
$result .= <<< EOT
<div class="ms-vmenublockheader"><h3 class="t">$subject</h3>
</div>
EOT;
}
$result .= <<< EOT

        <div class="ms-vmenublockcontent">$vmenu
</div>
</div>
EOT;
